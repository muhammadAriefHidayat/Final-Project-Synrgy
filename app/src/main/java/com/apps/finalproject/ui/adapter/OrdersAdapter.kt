...
class OrdersAdapter(val itemOrders: DataOrders, val context: Context) : Item<GroupieViewHolder>() {


    override fun bind(viewHolder: GroupieViewHolder, position: Int) {
        val imge = viewHolder.itemView.findViewById<ImageView>(R.id.img_kurir_item)
        val idDrawable = getDrawableInt(context, "itemOrders")
        Glide.with(context)
            .load(idDrawable)
            .placeholder(R.drawable.home)
            .into(imge);
        viewHolder.itemView.findViewById<TextView>(R.id.tv_kurir_name).text = "itemOrders"
    }
